...
private:
	...
	void publishRTMsg() {
		ros::Publisher joint_pub = nh_.advertise<sensor_msgs::JointState>(
				"joint_states", 1);
        ros::Publisher joint_v_pub = nh_.advertise<sensor_msgs::JointState>(
                "joint_voltage", 1);
		...
		while (ros::ok()) {
			...
			sensor_msgs::JointState joint_v_msg;
			joint_msg.name = robot_.getJointNames();
            joint_v_msg.name = robot_.getJointNames();
...
//            //joint voltages
//            joint_v_msg.header.stamp = joint_msg.header.stamp;
//            joint_v_msg.effort = robot_.rt_interface_->robot_state_->getVActual();
//            joint_v_pub.publish(joint_v_msg);
//			ROS_INFO_STREAM("Vol  = "<<joint_v_msg.effort[0]<<" "<<joint_v_msg.effort[1]<<" "<<joint_v_msg.effort[2]<<" "
//									 <<joint_v_msg.effort[3]<<" "<<joint_v_msg.effort[4]<<" "<<joint_v_msg.effort[5]<<" ");
